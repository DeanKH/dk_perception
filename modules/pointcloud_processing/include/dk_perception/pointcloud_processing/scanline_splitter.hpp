...
namespace dklib::perception::pointcloud_processing
{
template<typename PointT = pcl::PointXYZ>
class ScanlineSplitter
{
public:
  enum class SplitAxis { kX = 0, kY, kUnknown, };

  std::vector<pcl::PointIndices::Ptr> splitByScanLines(
    ...
    SplitAxis axis)
  {
    ...
    auto func =
      [](const typename pcl::PointCloud<PointT>::Ptr & pointcloud, const float min_value,
        const float max_value, float scan_interval,
        const std::string & axis_field_name)
      {
        std::vector<pcl::PointIndices::Ptr> indices_vec;

        for (float scan_current = min_value; scan_current < max_value;
          scan_current += scan_interval)
        {
...
private:
  static pcl::PointIndices::Ptr splitAlongAxis(
